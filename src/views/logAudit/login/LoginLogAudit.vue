<template>
  <div class="p-4">
    <el-card style="background-color: rgba(255,255,255,0.8)">
      ...
      <el-card style="margin-top: 2vh" header="登录日志列表">
        <el-table :data="paginatedData" border class="my-4" stripe highlight-current-row>
          ...
          <el-table-column prop="device" width="200">
            <template #header>
              <span style="font-weight: bolder;font-size: 15px;color: black">device信息</span>
            </template>
            <template #default="{ row }">
              <el-popover
                  placement="top"
              ...
              >
                <template #reference>
                  <span class="ellipsis">{{ row.device }}</span>
                </template>
              </el-popover>
            </template>
          </el-table-column>

          <el-table-column prop="location" width="150">
            <template #header>
              <span style="font-weight: bolder;font-size: 15px;color: black">登录地区</span>
            </template>
            <template #default="{ row }">
              <el-popover
                  placement="top"
              ...
              >
                <template #reference>
                  <el-tag type="info">{{ row.location }}</el-tag>
                </template>
              </el-popover>
            </template>
          </el-table-column>
          ...
          <el-table-column prop="status" width="100">
            <template #header>
              <span style="font-weight: bolder;font-size: 15px;color: black">是否成功</span>
            </template>
            <template #default="{ row }">
...
import {mockLoginLogs} from "../../../mock/log.ts";
